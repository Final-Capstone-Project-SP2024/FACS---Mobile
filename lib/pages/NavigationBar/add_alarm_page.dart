...
class _AddAlarmPageState extends State<AddAlarmPage> {
      ...
      if (response != null) {
        if (response.containsKey('data')) {
          ...
          for (var locationData in data) {
            ...
            locations.add({'locationId': locationId, 'locationName': locationName});
          }
          setState(() {
  ...
  void _navigateToCameraPage() async {
    ...
    if (capturedMedia != null && capturedMedia is List<File?> && capturedMedia.length == 2) {
      setState(() {
  ...
  Future<void> _sendAlert(File? imageFile, File? videoFile, String cameraId, int fireDetection) async {
    try {
      if ((imageFile != null && imageFile is File) || (videoFile != null && videoFile is File)) {
        List<int> imageData = [];
        List<int> videoData = [];
        ...
        await CameraServices.sendAlert(imageData, videoData, cameraId, fireDetection);
      } else {
        print('Error: Either imageFile or videoFile must be provided and must be of type File');
      }
    } catch (e) {
          ...
          children: [
            ...
            Padding(
              ...
              child: DropdownButton<String>(
                ...
                items: _locations.map<DropdownMenuItem<String>>((Map<String, String> location) {
